...
// SwapToken contract: Facilitates token swapping between NewToken and FlowToken
pub contract SwapToken {
    ...
    // Function to swap tokens between NewToken and FlowToken
    pub fun swapTokens(signer: AuthAccount, swapAmount: UFix64) {

        // Borrow NewToken and FlowToken vaults from the signer's storage
        let NewTokenVault = signer.borrow<&NewToken.Vault>(from: /storage/VaultStorage)
            ?? panic("Could not borrow NewToken Vault from signer")

        let flowVault = signer.borrow<&FlowToken.Vault>(from: /storage/FlowVault)
            ?? panic("Could not borrow FlowToken Vault from signer")  

        // Borrow Minter capability from NewToken
        let minterRef = signer.getCapability<&NewToken.Minter>(/public/Minter).borrow()
            ?? panic("Could not borrow reference to NewToken Minter")
        ...
        // Mint new NewTokens and deposit them to the vault
        let newNewTokenVault <- minterRef.mintToken(amount: mintedAmount)
        NewTokenVault.deposit(from: <-newNewTokenVault)
